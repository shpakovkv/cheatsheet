...
То же самое можно провернуть и через openocd.
...
Запускает сервер с постоянной связью с программатором (в нашем случае ST-Link V2) в монопольном режиме (устройство быдут недоступно для других приложений). Связываясь с сервером и отправляя ему команды можно считывать информацию из МК, прошивать его, перезагружать и т.д. 
...
Файл contrib/60-openocd.rules добавить в /etc/udev/rules.d


Настройка и запуск сервера openocd
----------------------------------

Параметр hla_vid_pid в конфиге интерфейса (stlink.cfg) это пара значений (idVendor idProduct). Их можно увидеть, подключив программатор к компу и запустив команду lsusb. Нужно заменить на актуальные.
```
...
Лечение: в файле tool-openocd\scripts\interface\stlink.cfg указать серийный номер в явном виде 
```
...
# стирание старой прошивки (помогает избежать наожиданных багов в работе МК) 
...
Если ноги SWD интерфейса сконфигурированы под другие нужды, то зажать reset  на МК и держать до ввода команды reset init (reset halt), отпустить сразу перед нажатием на enter или одновременно.
...
после запуска openocd gdb сервера в настройках CubeIDE->Debugg Configurations->Debugger установить:
* [x] Connect to remote GDB server
* Host name = localhost
* Port number = 3333
* Debug Probe = ST-LINK (OpenOCD)

##### Настройки Blue Pill STM32F103C8T6:
...
Программируя STM32 придется работать со строками в стиле Си (не Си++). Основные команды
